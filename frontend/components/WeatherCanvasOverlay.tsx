...
const ICE_RAMP: ColorStop[] = [
  [0.00,  80, 160, 255], [0.10, 140, 210, 255], [0.30,  80, 200, 200],
  [0.50, 100, 220, 100], [0.70, 230, 220,  40], [0.80, 240, 170,  20],
  [0.90, 210,  80,  30], [0.95, 160,  30,  70], [1.00, 255, 255, 255],
];
...
function buildColorLUT(mode: string, vMin: number, vMax: number): Uint8Array {
  ...
  for (let i = 0; i < LUT_SIZE; i++) {
    ...
    lut[off + 1] = g;
    lut[off + 2] = b;
    if (mode === 'ice' && value < 0.05) {
      // Gradual fade-in: 0% at <=0.1%, full alpha at 5%
      lut[off + 3] = value <= 0.001 ? 0 : Math.round(baseAlpha * ((value - 0.001) / 0.049));
